...
Модифицируем роль для сбора расширенной информации о запущенных службах, включая полное описание (Description) и другие свойства. Вот обновленная версия:
            ...
            {%- for info_result in detailed_service_info.results -%}
              {%- if info_result.item.name == service.name and info_result.stdout != '' -%}
                ...
                {%- if service_detail.update({
                  ...
                  'memory_usage': (properties.MemoryCurrent | default(0) | int // 1024) | string + ' KB',
...
- name: Add CSV header (only once)
  lineinfile:
    path: "{{ report_path }}/{{ output_filename }}.csv"
    line: "Hostname,Primary IP,All IPs,Distribution,Version,Architecture,CPU Model,CPU Cores,CPU Sockets,CPU Threads,CPU Topology,CPU Architecture,Memory (MB),Swap (MB),Virtualization,Disks,Running Services,Service Details"
    create: yes
    insertbefore: BOF
  delegate_to: localhost
  run_once: true
...
1. Расширенная информация о службах для systemd

· Description - полное описание службы
· LoadState - состояние загрузки службы
· ActiveState - активное состояние службы
· SubState - дополнительное состояние службы
· MainPID - основной PID процесса службы
· MemoryCurrent - использование памяти службой
· ExecMainStartTimestamp - время запуска службы
· FragmentPath - путь к файлу конфигурации службы

2. Улучшенная обработка для SysV init

· Более надежное извлечение описаний из init скриптов
· Попытка извлечь описание из комментариев в скриптах
· Структурированная информация о службах

3. Новый формат вывода в CSV

· Running Services - список имен служб через точку с запятой
· Service Details - расширенная информация о каждой службе в формате:
  ```
  Name: nginx; Description: A high performance web server; Status: active; PID: 1234; Memory: 10240 KB; Config: /lib/systemd/system/nginx.service
  ```

4. Новая переменная конфигурации

· collect_detailed_service_info - управляет сбором расширенной информации о службах

Пример вывода в CSV:

```
Hostname,Primary IP,...,Running Services,Service Details
"server1","192.168.1.10",...,"nginx;docker;ssh","Name: nginx; Description: A high performance web server; Status: active; PID: 1234; Memory: 10240 KB; Config: /lib/systemd/system/nginx.service| Name: docker; Description: Docker Application Container Engine; Status: active; PID: 5678; Memory: 20480 KB; Config: /lib/systemd/system/docker.service| Name: ssh; Description: OpenBSD Secure Shell server; Status: active; PID: 9012; Memory: 5120 KB; Config: /lib/systemd/system/ssh.service"
```
...
```yaml
---
- name: Collect comprehensive server information with detailed service info
  hosts: all
  become: yes
  gather_facts: no
  roles:
  ...
  vars:
    ...
    output_filename: "all_servers_detailed_inventory"
    collect_service_descriptions: true
    collect_detailed_service_info: true
    encrypt_report: false
```
...
```bash
# Сбор информации с расширенными данными о службах
ansible-playbook -i hosts.ini playbook.yml

# Только базовая информация о службах (для ускорения)
ansible-playbook -i hosts.ini playbook.yml -e "collect_detailed_service_info=false"
```

Теперь отчет будет содержать полную расширенную информацию о каждой запущенной службе, включая описание, состояние, PID, использование памяти и путь к конфигурационному файлу.

Модифицируем роль для отображения оперативной памяти в ГБ вместо МБ. Вот обновленная версия:
            ...
            {%- for info_result in detailed_service_info.results -%}
              {%- if info_result.item.name == service.name and info_result.stdout != '' -%}
                ...
                {%- for line in info_result.stdout.split('\n') -%}
                  ...
                  {%- endif -%}
                {%- endfor -%}
                {%- set memory_mb = (properties.MemoryCurrent | default(0) | int // 1024) -%}
                {%- set memory_gb = (memory_mb / 1024) | round(2) -%}
                {%- if service_detail.update({
                  'description': properties.Description | default('No description'),
                  'load_state': properties.LoadState | default('unknown'),
                  'active_state': properties.ActiveState | default('unknown'),
...
1. Конвертация памяти в ГБ

· Добавлена новая задача Convert memory to GB для преобразования МБ в ГБ
· Оперативная память: memory_gb: "{{ (ansible_memtotal_mb | default(0) | int / 1024) | round(2) }}"
· Swap память: swap_gb: "{{ (ansible_swaptotal_mb | default(0) | int / 1024) | round(2) }}"

2. Обновление переменных памяти

· В задаче Set safe facts with defaults заменены:
  · safe_memory: "{{ memory_gb }}"
  · safe_swap: "{{ swap_gb }}"

3. Обновление заголовков CSV

· Заголовок изменен с Memory (MB),Swap (MB) на Memory (GB),Swap (GB)

4. Конвертация памяти служб в ГБ

· В задаче Parse detailed service information добавлена конвертация памяти служб:
  ```yaml
  {%- set memory_mb = (properties.MemoryCurrent | default(0) | int // 1024) -%}
  {%- set memory_gb = (memory_mb / 1024) | round(2) -%}
  'memory_usage': memory_gb | string + ' GB'
...
```
Hostname,Primary IP,...,Memory (GB),Swap (GB),...
"server1","192.168.1.10",...,"16.0","2.0",...
```

Пример вывода информации о службах:

```
Name: nginx; Description: A high performance web server; Status: active; PID: 1234; Memory: 0.01 GB; Config: /lib/systemd/system/nginx.service
```

Преимущества изменений:

1. Более читаемый формат - ГБ вместо МБ для больших объемов памяти
2. Согласованность единиц измерения - все объемы данных теперь в ГБ
3. Округление до двух знаков - для удобства восприятия
4. Автоматическая конвертация - как для системной памяти, так и для памяти служб

Теперь отчет будет отображать все объемы памяти в гигабайтах, что делает его более читаемым и понятным, особенно для серверов с большим объемом оперативной памяти.