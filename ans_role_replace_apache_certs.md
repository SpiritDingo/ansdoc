...
Проблема в том, что при формировании переменной local_cert_path в многострочном блоке Jinja2 в неё попадают лишние пробелы и символ перевода строки. В вашем случае путь начинается с двух пробелов и заканчивается \n, из-за чего задача stat не может найти файл, хотя физически он существует.

Исправление

Замените блок "Определить полные локальные пути к исходным файлам (с нормализацией)" в файле tasks/main.yml на следующий код, который использует тернарные операторы и фильтр trim для гарантированного удаления лишних пробелов и переносов строк:

```yaml
- name: Определить полные локальные пути к исходным файлам (с нормализацией)
  set_fact:
    local_cert_path: "{{ (apache_cert_src.startswith('/')) | ternary(apache_cert_src, (apache_cert_src.startswith('files/')) | ternary(role_path + '/' + apache_cert_src, role_path + '/files/' + apache_cert_src)) | trim }}"
    local_key_path: "{{ (apache_key_src.startswith('/')) | ternary(apache_key_src, (apache_key_src.startswith('files/')) | ternary(role_path + '/' + apache_key_src, role_path + '/files/' + apache_key_src)) | trim }}"
    local_chain_path: "{{ (apache_chain_src != '') | ternary((apache_chain_src.startswith('/')) | ternary(apache_chain_src, (apache_chain_src.startswith('files/')) | ternary(role_path + '/' + apache_chain_src, role_path + '/files/' + apache_chain_src)), '') | trim }}"
  run_once: true
  tags: [always]
```

Этот вариант компактнее, не создаёт лишних пробелов и гарантирует чистоту путей.

Почему это работает

· Тернарный оператор (условие) | ternary(значение_если_истина, значение_если_ложь) заменяет многострочный if/elif/else.
· Фильтр trim удаляет пробелы и символы новой строки в начале и конце строки.
· Все части склеиваются без лишних отступов, так как выражение записано в одну строку.

Дополнительная рекомендация

После замены блока снова запустите плейбук с тегом debug, чтобы убедиться, что пути выводятся корректно:

```bash
ansible-playbook your_playbook.yml --tags debug
```

Вы должны увидеть, что local_cert_path теперь содержит чистый путь без начальных пробелов и \n.

Если ошибка persist, проверьте, не содержит ли сама переменная apache_cert_src случайных пробелов (например, задана с пробелом в конце). В таком случае добавьте | trim и к ней:

```yaml
apache_cert_src: "{{ apache_cert_src | trim }}"
```

но обычно это избыточно, если переменная задана корректно.

Полный исправленный блок в контексте роли

Если вы хотите заменить только проблемный участок, вышеприведённый код полностью решает задачу. Остальная часть роли остаётся без изменений.


Ошибка возникает из-за некорректного синтаксиса среза списка в Jinja2 внутри выражения {{ ... }}. Вместо list[keep_backups:] нужно использовать другой подход, например, через map и list или разделить логику на несколько задач. Ниже приведён исправленный блок задачи "Очистить ротацию старых бэкапов", который нужно заменить в файле tasks/main.yml.

Исправленный фрагмент (замените существующий блок always)
  ...
  always:
    - name: Очистить ротацию старых бэкапов (оставить только последние N)
      block:
        ...
        - name: Отсортировать бэкапы по времени (новые первыми)
          set_fact:
            sorted_backups: "{{ all_backups.files | sort(attribute='mtime', reverse=True) }}"
          when: all_backups.files | length > keep_backups

        - name: Определить файлы для удаления (старше последних keep_backups)
          set_fact:
            files_to_delete: "{{ sorted_backups[keep_backups:] | map(attribute='path') | list }}"
          when: all_backups.files | length > keep_backups

        - name: Удалить старые бэкапы
          file:
            path: "{{ item }}"
            state: absent
          loop: "{{ files_to_delete }}"
          when: files_to_delete | default([]) | length > 0
      tags: [backup, cleanup]
```

Почему это работает

· Сначала находим все файлы бэкапов с помощью модуля find.
· Сортируем их по времени модификации (новые в начале) и сохраняем в sorted_backups.
· Берём срез [keep_backups:] – это файлы, которые нужно удалить (старше последних N).
· Преобразуем их в список путей с помощью map(attribute='path').
· Удаляем каждый файл через loop.

Такой подход не вызывает синтаксических ошибок и надёжно работает в Ansible.

Полный исправленный файл tasks/main.yml (только блок always)

Если вы хотите видеть весь контекст, вставьте приведённый выше код вместо существующего блока always (начиная со строки always: и до конца файла или до соответствующего закрывающего блока). Остальная часть роли остаётся без изменений.

После замены запустите плейбук заново – ошибка должна исчезнуть.