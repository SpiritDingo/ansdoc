...
Добавим сбор IP-адресов и информации о запущенных службах по фильтру в Ansible роль. Вот обновленная версия:
...
```yaml
---
- name: Collect comprehensive server information
  hosts: all
  become: yes
  gather_facts: no
  roles:
  ...
  vars:
    ...
    service_filter:
      ...
      - "mongod"
    report_path: "/tmp/server_reports"
    output_filename: "all_servers_comprehensive_inventory"
    encrypt_report: false
...
· Основной IP - из ansible_default_ipv4.address
· Все IP-адреса - из ansible_all_ipv4_addresses (если include_all_ips: true)
· В отчете два столбца: Primary IP и All IPs

2. Сбор информации о службах

· Использует модуль service_facts для сбора информации о службах
· Фильтрует только запущенные службы (state: 'running')
· Поддерживает фильтрацию по service_filter
· Имеет fallback методы для systemd и SysV init систем

3. Обновленный CSV формат

Новые столбцы в отчете:

· Primary IP - основной IP-адрес сервера
· All IPs - все IP-адреса сервера (через точку с запятой)
· Running Services - список запущенных служб по фильтру

4. Настройки фильтров

· service_filter - список служб для мониторинга
· preferred_interface - предпочтительный интерфейс для основного IP
· include_all_ips - включать ли все IP-адреса в отчет
...
# С кастомными фильтрами служб
ansible-playbook -i hosts.ini playbook.yml \
  -e 'service_filter=["nginx","mysql","docker","ssh"]'

# Только основной IP (без всех адресов)
ansible-playbook -i hosts.ini playbook.yml \
  -e 'include_all_ips=false'
```
...
Добавим сбор информации о количестве CPU и полном названии модели процессора. Вот модифицированная роль:
...
1. Детальная информация о CPU

Новые переменные:

· safe_processor_count - количество процессоров (сокетов)
· safe_processor_threads_per_core - количество потоков на ядро
· safe_processor_vcpus - общее количество vCPU
· safe_cpu_arch_details - детали архитектуры CPU
· safe_cpu_topology - топология CPU (сокеты, ядра, потоки)

2. Улучшенное определение модели CPU

· Использует lscpu для получения точного названия модели
· Fallback на стандартные факты Ansible если lscpu недоступен
· Обработка разных форматов вывода

3. Новые столбцы в CSV отчете

· CPU Model - полное название модели процессора
· CPU Cores - количество ядер
· CPU Sockets - количество сокетов/процессоров
· CPU Threads - общее количество потоков (vCPU)
· CPU Topology - топология (сокеты × ядра × потоки)
· CPU Architecture - детали архитектуры

4. Команды для сбора информации о CPU

· lscpu - основная команда для детальной информации
· Обработка различных форматов вывода
· Резервные методы если lscpu недоступен

Пример вывода в CSV:

```
Hostname,Primary IP,All IPs,Distribution,Version,Architecture,CPU Model,CPU Cores,CPU Sockets,CPU Threads,CPU Topology,CPU Architecture,Memory (MB),Swap (MB),Virtualization,Disks,Running Services,Installed Software
"server1","192.168.1.10","192.168.1.10; 10.0.0.10","Ubuntu","22.04","x86_64","Intel(R) Xeon(R) CPU E5-2680 v4 @ 2.40GHz","8","2","16","Sockets:2;Cores per socket:4;Threads per core:2","x86_64;64-bit;Little Endian","16384","2048","kvm/guest","/:50.0GB; /data:100.0GB","nginx;docker;ssh","python3-3.10.6; docker-20.10.12; nginx-1.18.0"
```
...
Теперь отчет будет содержать полную детальную информацию о процессорах на каждом сервере, включая модель, количество сокетов, ядер и потоков, а также топологию CPU.
